...
function makeMesh(pts, extent) {
    ...
    // Voronoi object, which contains a cells object, and an edges object
    var vor = voronoi(pts, extent);

    // An array of the voronoi points, where vxs[i][0] is a point's x value, and vxs[i][1] is a point's y value,
    // and i is that point's index, which can be derived from the value of a vxids item
    var vxs = [];

    // Map of voronoi corners, where the key is the string 'x,y' for that point's x and y values, 
    // and the value is the the index of that point in vxs. This is only used when generating the
    // mesh so as to avoid duplicate entries in the vxs array
    var vxids = {};

    // Array of arrays, where each element contains the ids of the voronoi corner's adjacent to it
    // such that adj[i] would be correspond to the neighbors of vxs[i], where each element in adj[i]
    // is a list of array indexes for vxs e.g., vxs[adj[i][0]], vxs[adj[i][1]], vxs[adj[i][2]] etc...
    var adj = [];

    // An array of arrays, where each element contains information about an edget:
    // [
    //      The edge's left point index (where index would point to an element in vxs),
    //      The edge's right point index,
    //      An array [x, y] of the site to the left of the edge (perpendicular to the edge),
    //      An array [x, y] of the site to the right of the edge
    // ]
    var edges = [];

    // Delaunay triangulation: an array of arrays such tri[i] contains the coordinates
    // for the original point locations on the left and right of vxs[i]. After one
    // iteration, tri[i] will only have two entries, but eventually non-edge points
    // will have exactly three entries
    var tris = [];
    ...
    var mesh = {
        pts: pts,       // original points
        vor: vor,       // voronoi diagram object
        vxs: vxs,       // voronoi corners
        adj: adj,       // Adjacent voronoi corner indexes for each voronoi corner
        tris: tris,     // 
        edges: edges,   // Array of all voronoi edges and adjacent sites
        extent: extent
    };
...
function neighbours(mesh, index) {
...
function slope(mesh, direction) {
    var slopeMesh = mesh.map(function (x) {
...
function mountains(mesh, n, r) {
    ...
    // The way that this works is that n number of mountain points are randomly generated.
    // Then every point is looped through, and every 'mountain' is looped through for that point,
    // and the process of subtracting each mountain's coordinates from the point means that points
    // that are closest to a mountain are modified the least, and points that extend past the 2rr amount
    // are modified much more
    for (var i = 0; i < n; i++) {
    ...
    for (var i = 0; i < mesh.vxs.length; i++) {
        ...
        for (var j = 0; j < n; j++) {
            ...
            // Squaring the difference of the point position and the mountain position ensures it's a positive number
            // and the negative unary operator ensures that it is a negative number that we divide by 2r^2.
            // The result is that if (p[0] - m[0]) * (p[0] - m[0]) + (p[1] - m[1]) * (p[1] - m[1]) = x,
            // the closer that a point is to a mountain, the more that x approaches 0. When divided by 2r^2,
            // the closer x is to zero, the smaller the result is, with numbers that are larger quickly become much larger.
            // This number is always negative at this stage. If x / (2r^2) = y, performing Math.exp(y) means that the smaller
            // a y is, the larger the result is, and the larger y is, the smaller the result is. Squaring this finally means
            // that the result is just exaggerated even more. Points that are extremely close to a mountain will be larger
            // than points that are not quite as close to the mountain.
...
// Returns an array of indexes, where downs[i] is the downhill point's index
function downhill(h) {
...
// Assuming constant, uniform waterfall across the entire heightmap,
// get's the flux, or water flow rate according the the relationship
// between each point and it's downhill neighbor, for each point
function getFlux(h) {
    var dh = downhill(h); // An array of of each point's downhill neighbor
    var idxs = [];
    var flux = zero(h.mesh);
    for (var i = 0; i < h.length; i++) {
        idxs[i] = i;
        flux[i] = 1/h.length; // the base flux for every point
    }
    idxs.sort(function (a, b) { // Sort the points in order of highest to lowest
        return h[b] - h[a];
    });
    for (var i = 0; i < h.length; i++) { // since the points are sorted highest to lowest, we start with the highest point
        var j = idxs[i]; // j is the index in the downhill map dh
        if (dh[j] >= 0) { // As long as the downhill neighbor isn't the first element, add the uphill flux to the downhill flux
            flux[dh[j]] += flux[j];
        }
    }
    return flux;
...
// According to the distance of each point to it's neighbors (in regards to height),
// gets the slope for each point
function getSlope(h) {
    var dh = downhill(h);
...
// Returns the erosion rate for each point, which is the product of it's flux and slope, capped at 200
...
// Adjusts the height map for every point by the product of the amount
// given, and the erosion rate for each point
...
function drawPaths(svg, cls, p) {
    var paths = svg.selectAll('path.' + cls).data(p);
    paths.enter()
            .append('path')
            .classed(cls, true);
    paths.exit()
...
function visualizeSlopes(svg, render) {
    var h = render.h;
    var strokes = [];
    var r = 0.25 / Math.sqrt(h.length); // Stands for radius? This seems to be the default stroke length
    for (var i = 0; i < h.length; i++) {
        if (h[i] <= 0 || isnearedge(h.mesh, i)) continue; // Don't use this point if it's near the edge or below sea level
        var nbs = neighbours(h.mesh, i);
        nbs.push(i);
        var s = 0;
        var s2 = 0;
        ...
        s /= nbs.length;
        s2 /= nbs.length;
        if (Math.abs(s) < runif(0.1, 0.4)) continue; // If the slope is under this threshold, it's not steep enough to be drawn

        // The length of the stroke. It has the chance to be between 1 and 2 times the size of r, times...?
        var l = r * runif(1, 2) * (1 - 0.2 * Math.pow(Math.atan(s), 2)) * Math.exp(s2/100);
        var x = h.mesh.vxs[i][0];
        var y = h.mesh.vxs[i][1];

        // If the length (times slope) of the stroke will be longer that 2 times r,
        // break up the long stroke into a maximum of 4 shorter strokes
        if (Math.abs(l*s) > 2 * r) {
...
function doMap(svg, params) {
    ...
    var pointsSVG = d3.select('#points');
    var voronoiSVG = d3.select('#voronoi');

    // Set size of SVG
    var size = 1000;
    var svgs = [svg, pointsSVG, voronoiSVG];
    for (var i = 0; i < svgs.length; i++) {
        svgs[i].attr('width', size);
        svgs[i].attr('height', size * params.extent.height / params.extent.width);
        svgs[i].attr('viewBox', -1000 * params.extent.width/2 + ' ' + 
    ...
    var mesh = generateGoodMesh(params.npts, params.extent);
    var flatPTS = zero(mesh);
    var slopePTS = add(flatPTS, slope(flatPTS.mesh, randomVector(4)));
    var conePTS = add(flatPTS, cone(flatPTS.mesh, runif(-1, 1)));
    var mountainsPTS = add(flatPTS, mountains(flatPTS.mesh, 10, 0.09));
    var combinedPTS = add(flatPTS, slopePTS, conePTS, mountainsPTS);
    // var slopePTS = add(flatPTS, slope(flatPTS.mesh, [0,0]));
    visualizeVoronoi(voronoiSVG, combinedPTS);
    
    render.h = params.generator(params);
    placeCities(render);
    drawMap(svg, render);

    visualizePoints(pointsSVG, render.h.mesh.pts);
}
